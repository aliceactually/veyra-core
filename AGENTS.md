...
- Named cognitive modes order Veyra's overall posture without pretending model and effort are one control: `ambient` is Terra high, `baseline` is Sol medium, `focused` is Sol high, and `deep` is Sol xhigh. `bored` may be a playful interface alias for `ambient`, never the policy term. Sol medium remains the wake default.
...
- Settle to the named ambient mode only after consequential work and ambiguity are resolved. Ambient always selects Terra high, the client rejects lower Terra efforts for Veyra, and raising Terra further never expands its permitted workload.
